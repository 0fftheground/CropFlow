...
2026-07-18 已新增 `P4 Business Agent Runtime` 后端 MVP：Agent 可以基于真实 `PlantingPlan` 上下文查询任务和复核事项，并通过受控 Action、人工批准、PreAction 和审计复用既有业务 Service / `PlanOrchestrator`。DeepSeek V4-Pro 的真实 tool-calling 与 Runtime approval gate 已完成基础联调；目标数据库端到端 eval 和生产部署仍未完成。
...
Agent Runtime 本地默认使用 deterministic `scripted` provider。真实模型配置和 API 见：
