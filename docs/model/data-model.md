...
# 15. Agent Runtime 运行态数据

Business Agent Runtime 使用独立的 operational persistence：
...
```text
1. 这些表不复制 PlantingPlan、FarmingTask、OperationPlan、Execution 或 ReviewRequest 状态。
2. Business Agent 每轮从现有业务表构建 Object View。
3. 写 Action 通过现有 Service / PlanOrchestrator 执行。
4. session / message / run / tool call / approval 使用带类型前缀的字符串 ID；业务实体继续沿用 bigint 主键。
5. 详细字段、执行链路和生产边界见 docs/architecture/agent-runtime.md。
```
