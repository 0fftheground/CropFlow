...
1. 当前后端实现仍按旧口径调用算法：后端组装 `weather_data`，算法返回 `stage_timeline` 和 `thermal_thresholds`。
2. 本文档描述的是建议收敛的 `vNext` 契约：算法侧只负责返回可执行的积温规则包，后端负责天气驱动下的积温累计、生育期推进和阶段日期推导。
3. 在代码与算法服务完成同步改造前，本文档不等于当前已上线实现，而是后续 handoff 的目标口径。
...
2. 当前版算法只负责阈值规则匹配，不直接返回完整 `stage_timeline`。
3. 当前版规则包必须足够让后端独立完成积温累计和生育期推进。
4. 如后续需要新增特殊修正规则，优先通过追加字段扩展，不直接改变现有字段语义。
